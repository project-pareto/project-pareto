...
def solve_model(model, solver='gurobi', time_limit=360, abs_gap=0.0, mip_flag=False, tee=False, status_check=True):

    disjunctions = [d for d in model.component_data_objects(ctype=gdp.Disjunct, descend_into=True)]
...
def add_feasibility_cut(model, constraint_list):
    ...
    constraint_list.add(expr= sum(getattr(model, var)[index] for var, index in zeros.items()) + sum(1 - getattr(model, var)[index] for var, index in ones.items()) >= 1)
...
def integer_cut_decomposition(model:pyo.ConcreteModel, master_solver='gurobi', subproblem_solver='gurobi', time_limit=360, abs_gap=0.0, tee=False, 
                              iter_limit=100, rtol=1e-3, subproblem_warmstart=False, enable_status_outputs=False):
    # Start algorithm
    ...
    master = solve_model(master, solver=master_solver, time_limit=time_limit, abs_gap=abs_gap, mip_flag=True, tee=tee)
    ...
    while k < iter_limit:
        ...
        subproblem = solve_model(subproblem, solver=subproblem_solver, time_limit=time_limit, abs_gap=abs_gap, mip_flag=False, tee=tee, status_check=True)
        ...
        if subproblem.termination != 'infeasible':
            ...
            if abs_gap == 0.0 and master_solver == 'gurobi' and subproblem_solver == 'gurobi':
        ...
        if master.termination == 'infeasible':
            if best is None:
                if enable_status_outputs:
                    print('Master problem became infeasible. No solution found. Returning infeasible master')
                return master
            if enable_status_outputs:
