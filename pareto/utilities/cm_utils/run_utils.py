...
from pareto.utilities.cm_utils.gen_utils import report_results_to_excel, obj_fix, terminate, alter_nonlinear_cons
...
def plot_comparisons(values, sorted_total_costs):
    ...
    plot_titles = ["Arc cost", "Disposal cost", "Freshwater cost", 
                   "Treatment cost", "Storage cost", "Storage revenue", 
                   "Treatment revenue", '\nTotal cost']
    ...
    for plot_row in range(2):
        for plot_col in range(4):
            ...
            cost_type_values = [(node_num, values[node_num][current_title]) for node_num in values]
            sorted_station_values = sorted(cost_type_values, key=lambda x: sorted_total_costs_dict[x[0]])

            plot_compare_nodes(sorted_station_values, current_title, axs[plot_row][plot_col])
...
def change_piping_connection(data, base, old, new, pipe_in):
    ...
    if pipe_in:
        ...
        data['PipelineOperationalCost'][(new, base)] = data['PipelineOperationalCost'][(old, base)]
        data['PipelineOperationalCost'][(old, base)] = 0

        #edit initial pipeline capacity data
        data['InitialPipelineCapacity'][new, base] = data['InitialPipelineCapacity'][old, base]
    ...
    #case for when the arc that is being changed is being piped out of the base
        ...
        #edit operational cost data
        data['PipelineOperationalCost'][(base, new)] = data['PipelineOperationalCost'][(base, old)]
        data['PipelineOperationalCost'][(base, old)] = 0

        #edit initial pipeline capacity data
        data['InitialPipelineCapacity'][base, new] = data['InitialPipelineCapacity'][base, old]
...
def node_rerun(df_sets, df_parameters, treatment_site='R01', max_iterations=3000):
    ...
    for node in (df_sets['NetworkNodes'].tolist()):
        ...
        new_param_data = change_piping_connection(df_parameters, treatment_site, prev_node, node, pipe_in=True)
    ...
    sorted_costs_nozero = [(value, total_cost) for (value, total_cost) in sorted_costs if total_cost > 1e-4]
...
def truck_to_piping(min_node, models, data):
    ...
    trucked_arcs = set([(aOUT,aIN,aTP) for (aOUT,aIN,aTP,t) in list(min_model.v_F.keys()) 
                        if (aOUT,aIN,aTP,t)[2] == 'Trucked' and min_model.v_F[(aOUT,aIN,aTP,t)].value > 1e-2])
...
def truck_to_piping_run(df_sets, df_parameters, min_node, models, treatment_site, max_iterations = 3000):
    """
    ...
    new_param_data1 = change_piping_connection(df_parameters, treatment_site, prev_node, min_node, pipe_in=True)

    # changing the trucking to a piping arc if required
    new_param_data2, solve_choice, trucked_arcs = truck_to_piping(min_node, models, new_param_data1)
...
def solving(model, 
    ...
    if not term_cond:
        print('Linear model is infeasibility. This likely means the production flow \
    ...
    if term_cond == True:
        ...
        value_strings = ["Arc cost", "Disposal cost", "Freshwater cost",
                         "Treatment cost", "Storage cost", "Storage revenue",
                         "Treatment revenue", '\nTotal cost']
        
        values = [model.arc_cost, model.disp_cost, model.fresh_cost, 
                  model.treat_cost, model.stor_cost, model.stor_rev, 
                  model.treat_rev, model.obj()*1000]
