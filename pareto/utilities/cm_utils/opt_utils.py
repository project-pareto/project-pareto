...
def max_theoretical_recovery_flow(model, treatment_unit, desired_li_conc):
    ...
    for f,c in produced_flows_conc:
        ...
        if li_conc > desired_li_conc:
            ...
            ff = (cumulative_li - desired_li_conc*cumulative_f)/(desired_li_conc - c)
            # print('***', ff)
            cumulative_f += ff
            cumulative_li += ff*c
...
def max_theoretical_recovery_flow_opt(model, treatment_unit, desired_li_conc, tee=False):
    """ This function computes the largest flow possible to
    the treatment unit while still keeping the Li
    concentration above the desired level.
    ...
    mm.cumulative_F_con = \
        pyo.Constraint(expr = mm.cumulative_F == sum(mm.F[t] for t in mm.S) )
    mm.obj = pyo.Objective(expr = mm.cumulative_F, sense=pyo.maximize)
    mm.total_li = \
        pyo.Expression(expr = sum(mm.F[t]*model.p_CGen[t[0],'Li',t[1]] for t in mm.S))
    mm.quality_con = \
        pyo.Constraint(expr = mm.total_li >= desired_li_conc*mm.cumulative_F)
...
def max_recovery_with_infrastructure(data, tee=False):
    ...
    conclist = [model.MSconc, model.Pconc, model.Cconc, model.Sconc, model.Wconc, \
                model.TINconc, model.NTTWconc, model.NTCWconc, model.NTSrcconc, model.minconccon]
    flowlist = [model.Cflow, model.noinvflow, model.Sinv, model.Dflow, model.Wflow, model.TINflow, \
                model.NTTWflow, model.NTCWflow]
    ...
    status = opt.solve(model, tee=tee)
    pyo.assert_optimal_termination(status)
    print("Max lithium revenue with existing infrastructure:", pyo.value(model.treat_rev))
    return model
