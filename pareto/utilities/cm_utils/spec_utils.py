...
def set_processing(d, df_parameters):
    ...
    for n in d['s_NT']:
        ...
        k += 1
        d['s_A'].append((d['NT_set'][n][2],new_CW_disposal, 'Piped'))      # adding CW->K arcs
    ...
    # s_Qalpha
    d['s_Qalpha'] = {d['NT_set'][n][0]: [q for q in d['s_Q'] if d['s_Qalpha_int'][n, q] == 1] for n in d['s_NT']}
...
def parameter_processing(d, df_parameters):
    ...
    d['p_treatedIN'] = {d['NT_set'][n][1]: d['NT_set'][n][0] for n in d['s_NT']}    # treated water
    d['p_treatedIN'].update({d['NT_set'][n][2]: d['NT_set'][n][0] for n in d['s_NT']})  # concentrated water
    ...
    for (aIN,aOUT,aTP) in d['s_A']:
        if aTP == "Trucked":
            if (aIN,aOUT) in list(df_parameters['TruckingTime'].keys()):
                d['p_betaArc'][aIN,aOUT,aTP] == df_parameters['TruckingHourlyCost'][aIN]*df_parameters['TruckingTime'][aIN,aOUT]/110
