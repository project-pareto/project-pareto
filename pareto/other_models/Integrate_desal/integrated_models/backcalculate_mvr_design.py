...
def back_calculate_design(model, treatment_dict):
    ...
    for s in treatment_dict.keys():
        ...
        m.ann_fac = pyo.Param(initialize=pyo.value(model.m_treatment[s, 'T01'].annual_fac))
        ...
        m.preheater_capex = pyo.Var(initialize = pyo.value(model.global_preheater_capex[s]))
        m.preheater_capex.fix()
        
        m.compressor_capex = pyo.Var(initialize = pyo.value(model.global_compressor_capex[s]))
        m.compressor_capex.fix()
        
        #Design variables
        m.evap_area = pyo.Var(model.m_treatment[s, 'T01'].i, initialize = 1, domain = pyo.PositiveReals)
        ...
        def _evaporator_capex_con(m, i):
            fm = 1.8
            return m.evap_capex[i] == m.cepci_ratio*fm*1.218*pyo.exp(3.2362 - 0.0126*pyo.log(m.evap_area[i]*10.764) + 0.0244*(pyo.log(m.evap_area[i]*10.764))**2)
        m.evaporator_capex_con = pyo.Constraint(model.m_treatment[s,'T01'].i, rule = _evaporator_capex_con)
