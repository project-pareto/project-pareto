...
from pareto.other_models.Integrate_desal.models.qcp_desal import build_network
...
def add_desalination_cons(m, treatment_dict):
    ...
    @m.Constraint(m.desalination_nodes, 
                  m.s_T
        )
        ...
        return (
            sum(
                sum(
                    ...
                    for n in m.s_NTIN if n not in m.desalination_nodes
                )
                for t in m.s_T
            )
            * m.p_dt
...
def integrated_model_build(network_data, treatment_dict = {'R01_IN': 1}):
    ...
    for site in treatment_dict.keys():
        for t in m.m_network.s_T:
            m.m_treatment[site, t].flow_feed.fix(pyo.value(sum(m.m_network.v_F[:, site, :, t])*0.0013))
    ...
    m.global_preheater_capex = pyo.Var(m.m_network.desalination_nodes, domain = pyo.NonNegativeReals)
    m.global_compressor_capex = pyo.Var(m.m_network.desalination_nodes, domain = pyo.NonNegativeReals)
    m.CAPEX = pyo.Var(m.m_network.desalination_nodes, domain = pyo.NonNegativeReals)
    m.OPEX = pyo.Var(m.m_network.desalination_nodes, m.m_network.s_T, domain = pyo.NonNegativeReals)
    ...
    def _feed_flow_link(m, s, t):
        return m.m_treatment[s, t].flow_feed == sum(m.m_network.v_F[:, s, :, t])*0.0013
    m.feed_flow_link = pyo.Constraint(m.m_network.desalination_nodes, m.m_network.s_T, rule = _feed_flow_link)
    ...
    m.feed_conc_link = pyo.Constraint(m.m_network.desalination_nodes, m.m_network.s_T, rule = _feed_conc_link)
    ...
    m.recovery_fractions_link = pyo.Constraint(m.m_network.desalination_nodes, m.m_network.s_T, rule = _recovery_fractions_link)
    ...
    m.global_preheater_area_link = pyo.Constraint(m.m_network.desalination_nodes, m.m_network.s_T, rule = _global_ph_area_link)
    
    def _global_comp_capacity_link(m, s, t):
        return m.global_compressor_capex[s] >= m.m_treatment[s, t].compressor_capex
    m.global_compressor_capacity_link = pyo.Constraint(m.m_network.desalination_nodes, m.m_network.s_T, rule = _global_comp_capacity_link)
    ...
    m.opex_con = pyo.Constraint(m.m_network.desalination_nodes, m.m_network.s_T, rule = _opex_cal)
