...
def print_results(model):
    ...
    for t in model.s_T:
        for l in model.s_L:
            for l_hat in model.s_L:
                if (
                    model.v_F_Piped[l, l_hat, t].value != None
                    and model.v_F_Piped[l, l_hat, t].value > 0
                ):
                    v_F_Pipe_dict.append(
                        (l, l_hat, t, model.v_F_Piped[l, l_hat, t].value)
                    )
                    print(
                        model.v_F_Piped[l, l_hat, t],
                        "=",
    ...
    for t in model.s_T:
        for l in model.s_L:
            for l_hat in model.s_L:
                if (
                    model.v_F_Trucked[l, l_hat, t].value != None
                    and model.v_F_Trucked[l, l_hat, t].value > 0
                ):
                    v_F_Trucked_dict.append(
                        (l, l_hat, t, model.v_F_Trucked[l, l_hat, t].value)
                    )
                    print(
                        model.v_F_Trucked[l, l_hat, t],
                        "=",
                        model.v_F_Trucked[l, l_hat, t].value,
                    )
    ...
    for t in model.s_T:
        for f in model.s_F:
            for p in model.s_CP:
                if (
                    model.v_F_Sourced[f, p, t].value != None
                    and model.v_F_Sourced[f, p, t].value > 0
                ):
                    v_F_Sourced_dict.append((f, p, t, model.v_F_Sourced[f, p, t].value))
                    print(
                        model.v_F_Sourced[f, p, t],
                        "=",
                        model.v_F_Sourced[f, p, t].value,
                    )
    ...
    for t in model.s_T:
        for p in model.s_CP:
            if (
                model.v_F_PadStorageIn[p, t].value != None
                and model.v_F_PadStorageIn[p, t].value > 0
            ):
                v_F_PadStorageIn_dict.append((p, t, model.v_F_PadStorageIn[p, t].value))
                print(
                    model.v_F_PadStorageIn[p, t],
                    "=",
                    model.v_F_PadStorageIn[p, t].value,
                )
    ...
    for t in model.s_T:
        for p in model.s_CP:
            if (
                model.v_F_PadStorageOut[p, t].value != None
                and model.v_F_PadStorageOut[p, t].value > 0
            ):
                v_F_PadStorageOut_dict.append(
                    (p, t, model.v_F_PadStorageOut[p, t].value)
                )
                print(
                    model.v_F_PadStorageOut[p, t],
                    "=",
                    model.v_F_PadStorageOut[p, t].value,
                )
    ...
    for t in model.s_T:
        for l in model.s_L:
            for l_hat in model.s_L:
                if (
                    model.v_C_Piped[l, l_hat, t].value != None
                    and model.v_C_Piped[l, l_hat, t].value > 0
                ):
                    v_C_Piped_dict.append(
                        (l, l_hat, t, model.v_C_Piped[l, l_hat, t].value)
                    )
                    print(
                        model.v_C_Piped[l, l_hat, t],
                        "=",
                        model.v_C_Piped[l, l_hat, t].value,
                    )
    ...
    for t in model.s_T:
        for l in model.s_L:
            for l_hat in model.s_L:
                if (
                    model.v_C_Trucked[l, l_hat, t].value != None
                    and model.v_C_Trucked[l, l_hat, t].value > 0
                ):
                    v_C_Trucked_dict.append(
                        (l, l_hat, t, model.v_C_Trucked[l, l_hat, t].value)
                    )
                    print(
                        model.v_C_Trucked[l, l_hat, t],
                        "=",
                        model.v_C_Trucked[l, l_hat, t].value,
                    )
    ...
    for t in model.s_T:
        for f in model.s_F:
            for p in model.s_CP:
                if (
                    model.v_C_Sourced[f, p, t].value != None
                    and model.v_C_Sourced[f, p, t].value > 0
                ):
                    v_C_Sourced_dict.append((f, p, t, model.v_C_Sourced[f, p, t].value))
                    print(
                        model.v_C_Sourced[f, p, t],
                        "=",
                        model.v_C_Sourced[f, p, t].value,
                    )
    ...
    for t in model.s_T:
        for k in model.s_K:
            if (
                model.v_C_Disposal[k, t].value != None
                and model.v_C_Disposal[k, t].value > 0
            ):
                v_C_Disposal_dict.append((k, t, model.v_C_Disposal[k, t].value))
    ...
    for t in model.s_T:
        for p in model.s_CP:
            if model.v_C_Reuse[p, t].value != None and model.v_C_Reuse[p, t].value > 0:
                v_C_Reuse_dict.append((p, t, model.v_C_Reuse[p, t].value))
    ...
    if model.v_C_Slack.value != None and model.v_C_Slack.value > 0:
        ...
        for t in model.s_T:
            for p in model.s_CP:
                if (
                    model.v_S_FracDemand[p, t].value != None
                    and model.v_S_FracDemand[p, t].value > 0
                ):
                    v_S_FracDemand_dict.append((p, t, model.v_S_FracDemand[p, t].value))
                    print(
                        model.v_S_FracDemand[p, t],
                        "=",
                        model.v_S_FracDemand[p, t].value,
                    )
        ...
        for t in model.s_T:
            for p in model.s_PP:
                if (
                    model.v_S_Production[p, t].value != None
                    and model.v_S_Production[p, t].value > 0
                ):
                    v_S_Production_dict.append((p, t, model.v_S_Production[p, t].value))
                    print(
                        model.v_S_Production[p, t],
                        "=",
                        model.v_S_Production[p, t].value,
                    )
        ...
        for t in model.s_T:
            for p in model.s_CP:
                if (
                    model.v_S_Flowback[p, t].value != None
                    and model.v_S_Flowback[p, t].value > 0
                ):
        ...
        for l in model.s_L:
            for l_tilde in model.s_L:
                if (
                    model.v_S_PipelineCapacity[l, l_tilde].value != None
                    and model.v_S_PipelineCapacity[l, l_tilde].value > 0
                ):
                    v_S_PipelineCapacity_dict.append(
                        (l, l_tilde, model.v_S_PipelineCapacity[l, l_tilde].value)
                    )
                    print(
                        model.v_S_PipelineCapacity[l, l_tilde],
                        "=",
                        model.v_S_PipelineCapacity[l, l_tilde].value,
                    )
        ...
        for s in model.s_S:
            if (
                model.v_S_StorageCapacity[s].value != None
                and model.v_S_StorageCapacity[s].value > 0
            ):
                v_S_StorageCapacity_dict.append((s, model.v_S_StorageCapacity[s].value))
                print(
                    model.v_S_StorageCapacity[s],
                    "=",
                    model.v_S_StorageCapacity[s].value,
                )
        ...
        for r in model.s_R:
            if (
                model.v_S_TreatmentCapacity[r].value != None
                and model.v_S_TreatmentCapacity[r].value > 0
            ):
                v_S_TreatmentCapacity_dict.append(
                    (r, model.v_S_TreatmentCapacity[r].value)
                )
                print(
                    model.v_S_TreatmentCapacity[r],
                    "=",
                    model.v_S_TreatmentCapacity[r].value,
                )
        ...
        for o in model.s_O:
            if (
                model.v_S_ReuseCapacity[o].value != None
                and model.v_S_ReuseCapacity[o].value > 0
            ):
        ...
        slacks_dict = {
            "v_S_FracDemand": v_S_FracDemand_dict,
    ...
    solution_dict = {
        "v_F_Pipe": v_F_Pipe_dict,
        "v_F_Trucked": v_F_Trucked_dict,
        "v_F_Sourced": v_F_Sourced_dict,
        "v_F_PadStorageIn": v_F_PadStorageIn_dict,
        "v_F_PadStorageOut": v_F_PadStorageOut_dict,
        "v_C_Piped": v_C_Piped_dict,
        "v_C_Trucked": v_C_Trucked_dict,
        "v_C_Sourced": v_C_Sourced_dict,
        "v_C_Disposal": v_C_Disposal_dict,
        "v_C_Reuse": v_C_Reuse_dict,
    }
