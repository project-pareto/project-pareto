...
# Notes:
# - Introduced new completions-to-completions trucking arc (CCT) to account
# for possible flowback reuse
# - Implemented a generic OPERATIONAL case study example (updated model sets,
# additional input data)
# - Implemented an initial formulation for production tank modeling
# (see updated documentation)

# Imports
from pyomo.environ import (Set, Param, Constraint,
                           ConcreteModel, Var, Reals,
...
# method to build operational produced water optimization model
def create_model(df_sets, df_parameters, default={}):
    model = ConcreteModel()
    model.config = CONFIG(default)
    # example of using config to construct different params/constraints
    if model.config.has_pipeline_constraints is True:
        model.st = Param(initialize=1)
    else:
        model.st = Param(initialize=10)

    # another example of config flags
    if model.config.production_tanks == ProdTank.single:
        model.eq_test = Param(initialize=100)
    elif model.config.production_tanks == ProdTank.aggregated:
        model.eq_test = Param(initialize=100000)
    else:
        raise Exception('storage type not supported')

    # Define sets #
    model.s_T = Set(initialize=df_sets['TimePeriods'],
                    doc='Time Periods', ordered=True)
    model.s_PP = Set(initialize=df_sets['ProductionPads'],
                     doc='Production Pads')
    model.s_CP = Set(initialize=df_sets['CompletionsPads'],
                     doc='Completions Pads')
    model.s_A = Set(initialize=df_sets['ProductionTanks'],
                    doc='Production Tanks')
    model.s_P = Set(initialize=(model.s_PP | model.s_CP), doc='Pads')
    model.s_F = Set(initialize=df_sets['FreshwaterSources'],
                    doc='Freshwater Sources')
    ...
    model.s_R = Set(initialize=df_sets['TreatmentSites'],
                    doc='Treatment Sites')
    ...
    model.s_L = Set(initialize=(model.s_P | model.s_F | model.s_K | model.s_S |
                                model.s_R | model.s_O | model.s_N),
                    doc='Locations')

    # COMMENT: Remove pipeline diameter,
    # storage capacity and injection capacity sets
    model.s_D = Set(initialize=['D0'], doc='Pipeline diameters')
    model.s_C = Set(initialize=['C0'], doc='Storage capacities')
    model.s_I = Set(initialize=['I0'],
    ...
    model.v_F_Piped = Var(model.s_L, model.s_L, model.s_T,
                          within=NonNegativeReals,
                          doc='Produced water quantity piped from '
                          'location l to location l [bbl/day]')

    model.v_F_Trucked = Var(model.s_L, model.s_L, model.s_T,
                            within=NonNegativeReals,
                            doc='Produced water quantity trucked '
                            'from location l to location l [bbl/day]')

    model.v_F_Sourced = Var(model.s_F, model.s_CP, model.s_T,
                            within=NonNegativeReals,
                            doc='Fresh water sourced from source f '
                            'to completions pad p [bbl/day]')

    if model.config.production_tanks == ProdTank.single:
        model.v_F_Drain = Var(model.s_P, model.s_A, model.s_T,
                              within=NonNegativeReals,
                              doc='Produced water drained from '
                              'production tank [bbl/day]')
        model.v_L_ProdTank = Var(model.s_P, model.s_A, model.s_T,
                                 within=NonNegativeReals,
                                 doc='Water level in production tank [bbl]')

    elif model.config.production_tanks == ProdTank.aggregated:
        model.v_F_PadStorageIn = Var(model.s_CP, model.s_T,
                                     within=NonNegativeReals,
                                     doc='Water put into completions '
                                     'pad storage [bbl/day]')
        model.v_F_PadStorageOut = Var(model.s_CP, model.s_T,
                                      within=NonNegativeReals,
                                      doc='Water from completions pad storage'
                                      ' used for fracturing [bbl/day]')
        model.v_F_Drain = Var(model.s_P, model.s_T,
                              within=NonNegativeReals,
                              doc='Produced water drained from '
                              'production tank [bbl/day]')
        model.v_L_ProdTank = Var(model.s_P, model.s_T, within=NonNegativeReals,
                                 doc='Water level in production tank [bbl]')
        model.v_L_PadStorage = Var(model.s_CP, model.s_T,
                                   within=NonNegativeReals,
                                   doc='Water level in completions '
                                   'pad storage [bbl]')

    else:
        raise Exception('storage type not supported')

    model.v_B_Production = Var(model.s_P, model.s_T,
                               within=NonNegativeReals,
                               doc='Produced water for transport '
                               'from pad [bbl/day]')

    model.v_L_Storage = Var(model.s_S, model.s_T,
                            within=NonNegativeReals,
                            doc='Water level at storage site [bbl]')

    model.v_C_Piped = Var(model.s_L, model.s_L, model.s_T,
                          within=NonNegativeReals,
                          doc='Cost of piping produced water from'
                          ' location l to location l [$/day]')
    model.v_C_Trucked = Var(model.s_L, model.s_L, model.s_T,
                            within=NonNegativeReals,
                            doc='Cost of trucking produced water from'
                            ' location l to location l [$/day]')
    model.v_C_Sourced = Var(model.s_F, model.s_CP, model.s_T,
                            within=NonNegativeReals,
                            doc='Cost of sourcing fresh water from '
                            'source f to completion pad p [$/day]')
    model.v_C_Disposal = Var(model.s_K, model.s_T,
                             within=NonNegativeReals,
                             doc='Cost of injecting produced water'
                             ' at disposal site [$/day]')
    model.v_C_Treatment = Var(model.s_R, model.s_T,
                              within=NonNegativeReals,
                              doc='Cost of treating produced '
                              'water at treatment site [$/day]')
    model.v_C_Reuse = Var(model.s_CP, model.s_T,
                          within=NonNegativeReals,
                          doc='Cost of reusing produced water'
                          ' at completions site [$/day]')
    model.v_C_Storage = Var(model.s_S, model.s_T,
                            within=NonNegativeReals,
                            doc='Cost of storing produced water '
                            'at storage site [$/day]')
    model.v_R_Storage = Var(model.s_S, model.s_T,
                            within=NonNegativeReals,
                            doc='Credit for retrieving stored '
                            'produced water from storage site [$/bbl]')
    model.v_C_TotalSourced = Var(within=NonNegativeReals,
                                 doc='Total cost of sourcing freshwater [$]')
    model.v_C_TotalDisposal = Var(within=NonNegativeReals,
                                  doc='Total cost of injecting '
                                  'produced water [$]')
    model.v_C_TotalTreatment = Var(within=NonNegativeReals,
                                   doc='Total cost of treating '
                                   'produced water [$]')
    model.v_C_TotalReuse = Var(within=NonNegativeReals,
                               doc='Total cost of reusing produced water [$]')
    model.v_C_TotalPiping = Var(within=NonNegativeReals,
                                doc='Total cost of piping produced water [$]')
    model.v_C_TotalStorage = Var(within=NonNegativeReals,
                                 doc='Total cost of storing'
                                 ' produced water [$]')
    model.v_C_TotalTrucking = Var(within=NonNegativeReals,
                                  doc='Total cost of trucking '
                                  'produced water [$]')
    model.v_C_Slack = Var(within=NonNegativeReals,
                          doc='Total cost of slack variables [$')
    model.v_R_TotalStorage = Var(within=NonNegativeReals,
                                 doc='Total credit for withdrawing '
                                 'produced water [$]')
    # COMMENT: Remove the disposal/storage/flow capacity variables
    model.v_D_Capacity = Var(model.s_K, within=NonNegativeReals,
                             doc='Disposal capacity at a disposal'
                             ' site [bbl/day]')
    model.v_X_Capacity = Var(model.s_S, within=NonNegativeReals,
                             doc='Storage capacity at '
                             'a storage site [bbl/day]')
    model.v_F_Capacity = Var(model.s_L, model.s_L,
                             within=NonNegativeReals,
                             doc='Flow capacity along pipeline arc [bbl/day]')

    # COMMENT: Remove the disposal/pipine/storage capital capacity variables
    model.v_C_DisposalCapEx = Var(within=NonNegativeReals,
                                  doc='Capital cost of constructing '
                                  'or expanding disposal capacity [$]')
    model.v_C_PipelineCapEx = Var(within=NonNegativeReals,
                                  doc='Capital cost of constructing '
                                  'or expanding piping capacity [$]')
    model.v_C_StorageCapEx = Var(within=NonNegativeReals,
                                 doc='Capital cost of constructing '
                                 'or expanding storage capacity [$]')
    model.v_S_FracDemand = Var(model.s_CP, model.s_T, within=NonNegativeReals,
                               doc='Slack variable to meet the completions '
                               'demand [bbl/day]')
    model.v_S_Production = Var(model.s_PP, model.s_T, within=NonNegativeReals,
                               doc='Slack variable to process the produced '
                               'water production [bbl/day]')
    model.v_S_Flowback = Var(model.s_CP, model.s_T, within=NonNegativeReals,
                             doc='Slack variable to proces flowback water '
                             'production [bbl/day]')
    model.v_S_PipelineCapacity = Var(model.s_L, model.s_L,
                                     within=NonNegativeReals,
                                     doc='Slack variable to provide necessary'
                                     ' pipeline capacity [bbl]')
    model.v_S_StorageCapacity = Var(model.s_S, within=NonNegativeReals,
                                    doc='Slack variable to provide necessary '
                                    'storage capacity [bbl]')
    model.v_S_DisposalCapacity = Var(model.s_K, within=NonNegativeReals,
                                     doc='Slack variable to provide necessary'
                                     ' disposal capacity [bbl/day]')
    model.v_S_TreatmentCapacity = Var(model.s_R, within=NonNegativeReals,
                                      doc='Slack variable to provide necessary'
                                      ' treatment capacity [bbl/weel]')
    model.v_S_ReuseCapacity = Var(model.s_O, within=NonNegativeReals,
                                  doc='Slack variable to provide necessary '
                                  'reuse capacity [bbl/day]')
    # Define binary variables #

    # COMMENT: Remove the binary pipeline/storage/disposal variables
    model.vb_y_Pipeline = Var(model.s_L, model.s_L, model.s_D, within=Binary,
                              doc='New pipeline installed between one location'
                              ' and another location with specific diameter')
    model.vb_y_Storage = Var(model.s_S, model.s_C, within=Binary,
                             doc='New or additional storage facility installed'
                             ' at storage site with specific storage capacity')
    model.vb_y_Disposal = Var(model.s_K, model.s_I, within=Binary,
                              doc='New or additional disposal facility '
                              'installed at disposal site with '
                              'specific injection capacity')
    model.vb_y_Flow = Var(model.s_L, model.s_L, model.s_T, within=Binary,
    ...
    if model.config.production_tanks == ProdTank.single:
        ...
        model.p_lambda_ProdTank     = Param(model.s_P,default=0,
                                initialize=InitialTankLevelTable, doc='Initial water level in equalized production tanks [bbl]')
    ...
    def CompletionsPadDemandBalanceRule(model,p,t):
        return model.p_gamma_Completions[p,t] == (sum(model.v_F_Piped[n,p,t] for n in model.s_N if model.p_NCA[n,p])
                                                ...
                                                + (model.v_F_PadStorageOut[p,t] if model.config.production_tanks == ProdTank.aggregated else 0)
                                                - (- model.v_F_PadStorageIn[p,t] if model.config.production_tanks == ProdTank.aggregated else 0)
    ...
    if model.config.production_tanks == ProdTank.aggregated:
...
def print_results(model):
    ...
    for t in model.s_T:
        for l in model.s_L:
            for l_hat in model.s_L:
                if (model.v_F_Piped[l, l_hat, t].value != None
                        and model.v_F_Piped[l, l_hat, t].value) > 0:
                    print(model.v_F_Piped[l, l_hat, t],
                          '=',
                          model.v_F_Piped[l, l_hat, t].value)
    ...
    if model.v_C_Slack.value != None and model.v_C_Slack.value > 0:
        print('!!!ATTENTION!!! One or several '
              'slack variables have been triggered!')
    else:
        print('all slack variables are zero')
