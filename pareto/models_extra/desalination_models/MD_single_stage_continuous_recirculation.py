...
def build():
    ...
    # properties
    m.fs.properties_hot_ch = props_NaCl.NaClParameterBlock()
    m.fs.properties_cold_ch = props_NaCl.NaClParameterBlock()
    ...
    m.fs.eq_recycle_ratio = Constraint(
        expr=sum(m.fs.feed.flow_mass_phase_comp[0, "Liq", j] for j in ["H2O", "NaCl"])
...
def set_operating_conditions(
    ...
    # heater
    heater_top_temperature = 273.15 + 60
...
def display_system(m):
    ...
    permeate_flow_mass = sum(
        m.fs.permeate.flow_mass_phase_comp[0, "Liq", j].value for j in ["H2O", "NaCl"]
    )
    permeate_mass_frac_TDS = (
        m.fs.permeate.flow_mass_phase_comp[0, "Liq", "NaCl"].value / permeate_flow_mass
    )
    print(
    ...
    reject_flow_mass = sum(
        m.fs.reject.flow_mass_phase_comp[0, "Liq", j].value for j in ["H2O", "NaCl"]
    )
    reject_mass_frac_TDS = (
        m.fs.reject.flow_mass_phase_comp[0, "Liq", "NaCl"].value / reject_flow_mass
    )
    print(
