...
def create_model(df_sets, df_parameters, default={}):
    ...
    def surrogateoperatingRule(model):
        constraint = model.v_C_TotalTreatment_surrogate == sum(
            sum( model.v_C_Treatment_site[r,t]/52 for r in model.s_R) for t in model.s_T
        )

        return process_constraint(constraint)

    model.surrogateOperatingCost = Constraint(
        rule=surrogateoperatingRule, doc="Total treatment cost"
    )
...
def water_quality(model):
    ...
    model.quality.v_Q_surrogate = Var(
        model.s_R,
        model.s_QC,
        model.s_T,
        within=NonNegativeReals,
    ...
    conversion_factor_quality = pyunits.convert_value(1, from_units=model.model_units["concentration"], to_units=model.model_units["g_per_L"])
   
 
    cap_lower_bound, cap_upper_bound = 0 * 7 *  conversion_factor, 0.5 * 7 *  conversion_factor  
    opex_lower_bound, opex_upper_bound = 0 , 200
    capex_lower_bound, capex_upper_bound = 0, 100
    energy_lower_bound, energy_upper_bound = 0, 200
    ...
    def scalingTreatment(b, r, t):
        if b.parent_block().p_chi_DesalinationSites[r]:
            return b.parent_block().v_T_Treatment_scaled[r, t] == conversion_factor * b.parent_block().v_F_TreatmentFeed[r, t]
        else:
            return Constraint.Skip
    model.quality.treatment_vol = Constraint(model.s_R, model.s_T, rule=scalingTreatment)

    def scalingTreatmentQuality(b, r, qc, t):
        if b.parent_block().p_chi_DesalinationSites[r]:
            return b.v_Q_surrogate[r, qc, t] == conversion_factor_quality * b.v_Q[r, qc, t]
        else:
            return Constraint.Skip
    model.quality.treatment_quality = Constraint(model.s_R, model.s_QC, model.s_T, rule=scalingTreatmentQuality)
        ...
        for qc in model.s_QC:
            for t in model.s_T:
                ...
                if model.p_chi_DesalinationSites[i]:
                    ...
                    model.quality.v_Q_surrogate[i, qc, t]
    #endregion            
