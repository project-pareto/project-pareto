...
def create_model(df_sets, df_parameters, default={}):
    ...
    conversion_factor = pyunits.convert_value(
        ...
        to_units=model.model_units["bbl_per_week"],
    )
    ...
    def treatmentSurrogate(model):
        return model.v_C_TotalTreatment_surrogate == sum(
            model.v_C_Treatment_site[i, t] / 52 for i in model.s_R for t in model.s_T
        )

    model.TotalTreatment_cost = Constraint(
    ...
    model.max_operating = Constraint(
        model.s_R, model.s_T, rule=treatmentOperationSurrogate, doc="Opex"
    )
